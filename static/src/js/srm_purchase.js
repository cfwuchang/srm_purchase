odoo.define('srm_purchase.purchase', function (require) {
    ...
    var getDeliveryData = function () {
        ...
        _.each($rows, function (tr) {
            ...
            var amount = arseFloat($row.find('span#qty_unship')[0].innerText);
            data[line_id] = amount;
            // if ($amount.length) {
    ...
    // //实时价格
    // $('div.aa').on('focusout', function (event,index) {
    //     var price_unit=event.target.value
    //     // var product_qty=this.value
    //     // var description=this.$('div.description').value
    //     // $(this).find("span.price_subtotal").html= '1222'  
    ...
    var dates_list = function () {
        var $rows = $('.price-unit-quote-line');
        var data = {};
        _.each($rows, function (div) {
            var $row = $(div);
            var line_id = $row.data('id');
            var $date_planned = $row.find('input.tate_quote');
            if ($date_planned.length) {
                var date_planned = $date_planned.val();
                data[line_id] = date_fun(Date.parse(new Date(date_planned)))
            }
        });
        return data;
    };
    
    var date_fun= function formatDate(date) {
