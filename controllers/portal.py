...
class PurchaseCustomerPortal(CustomerPortal):
    ...
    def portal_delivery_action_form(self, **kw):
        ...
        #     for line_id, datas in purchase_quote_data.items():
        #         all_data.append((1, int(line_id), {'quantity_done': datas}))
        #     if all_data:
        #         order_id.write({'move_ids_without_package': all_data})
        #         now_user = request.env['res.users'].sudo().browse(int(http.request.uid))
        #         body = "{} 提交了入库数量，提交时间：{}".format(now_user.name, now_date)
    ...
    def portal_purchase_quote_action(self, **kw):
        ...
        if action == 'submit':
            all_data = []
            purchase_quote_data = params.get('vals', {})
            quote_data = params.get('vals_date', {})
            for line_id, datas in purchase_quote_data.items():
                all_data.append((1, int(line_id), {'x_price': datas}))
            for line_id, datetime_str in quote_data.items():
                all_data.append((1, int(line_id), {'date_planned': datetime_str}))
            if all_data:
    ...
    def check_the_purchase_order_delivery_status(self, order):
        ...
        for each_line in order.order_line.sudo().filtered(lambda u: u.product_id.type != 'service'):
            if each_line.qty_received == each_line.product_qty:
                all = True
            if each_line.qty_received == 0:
                none = True
        if all is True:
            return 'all_delivery'
        elif none is True:
